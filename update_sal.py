...
class Ui_Dialog(object):
    ...
    def update_sal(self):
        try:
            ...
            var_titlesal = self.titlesalEdit.text()
            var_totalsal = eval(var_basedsal) - eval(var_deductionsal) - eval(var_penaltysal) - eval(var_taxsal) + eval(var_latesal) + eval(var_travelsal) + eval(var_titlesal)
            if "'" in var_salno or "'" in var_empno or "'" in var_basedsal or "'" in var_deductionsal or "'" in var_penaltysal or "'" in var_taxsal or "'" in var_latesal or "'" in var_travelsal or "'" in var_titlesal or var_empno=='' or var_saltime=='' or var_basedsal=='' or var_deductionsal=='' or var_penaltysal=='' or var_taxsal=='' or var_latesal=='' or var_travelsal=='' or var_titlesal=='':
                self.messagelabel.setText("输入有误，请重新输入！")
            else:
                ...
                if (rs == []):
                    ...
                    c.execute('update sal set empno={},saltime={},basedsal={},deductionsal={},penaltysal={},taxsal={},latesal={},travelsal={},titlesal={},totalsal={} where salno={}'.format(var_empno,var_saltime,var_basedsal,var_deductionsal,var_penaltysal,var_taxsal,var_latesal,var_travelsal,var_titlesal,var_totalsal,var_salno))  # 使用cursor进行各种操作
                    self.messagelabel.setText("")
                    conn.commit()
                    c.close()  # 关闭cursor
                    conn.close()
    ...
    def setupUi(self, Dialog):
        ...
        self.b_ok.clicked.connect(self.update_sal)
    ...
    def retranslateUi(self, Dialog):
        ...
        Dialog.setWindowTitle(_translate("Dialog", "根据工资号修改对应工资信息"))
